...
from ch7.cross_val_snips import PurgedKFold
from ch8.feat_imp_snips import get_test_data


class MyPipeline(Pipeline):
    """
    Custom override of fit method to handle given sample weights 
    """
...
def clf_hyper_fit(feat, label, t1, pipe_clf, search_params, scoring=None,
    ...
    """
    Overrides sklearns gridSearchCV method for grid search of parameters to incorporate PurgedKFold class
    """
    # Set defaut value for scoring
    # neg_log_loss preferred over f1 or accuracy as neg_log_loss takes into account how confident errors were
    # - ex: a miss with 90% certainty counts more than a miss with 55% certainty
    if scoring is None:
    ...
    if bagging[0] > 0:
        ...
        bag_est = best_pipe.fit(feat, label, sample_weight=fit_params[bag_est.base_estimator.steps[-1][0] + '__sample_weight'])
...
def demo():
    ...
    X, label = get_test_data(n_features=10, n_informative=5, n_redundant=0, n_samples=100)
    # print(X.head())
    # print(label.head())
    
    # hyper parameter fitting with given list of potential parameters
    name = 'svc'
    ...
    clf = clf_hyper_fit(X, label['bin'], t1=label['t1'], pipe_clf=pipe_clf, scoring='neg_log_loss',
                        ...
                        rnd_search_iter=0, n_jobs=-1, pct_embargo=0., **fit_params)
    print("Finished hyper fit with given parameters")
    print(clf.steps[0][1].get_params())
                        
    # hyper parameter fitting with list of potential parameters with log uniform distribution
    name = 'svc'
    ...
    print("Finished hyper fit with log uniform parameters")
    print(clf.steps[0][1].get_params())
